...
`
  ...
  },
  {
    ...
    "answer": `1. 새 노드 생성: 삽입할 노드를 메모리에 할당하고 필요한 데이터를 저장합니다. 
              2. 새 노드의 다음 노드(next) 포인터 설정: 새 노드의 다음 노드 포인터를 삽입할 위치의 노드(현재 노드)를 가리키도록 설정합니다.   
              3. 새 노드 삽입 이전 노드(prev)의 다음 노드 포인터 설정: 삽입할 위치 이전 노드의 다음 노드 포인터를 새 노드를 가리키도록 설정합니다. 
              4. 새 노드 삽입 후 노드(current)의 이전 노드 포인터 설정: 삽입된 노드의 이전 노드 포인터를 새 노드를 가리키도록 설정합니다. `
  },
  {
    "chapter": "8장",
